...
if [ -z "$JDK" -a -s "$HOME/.AndroidStudio3.4/config/studio.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.AndroidStudio3.4/config/studio.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
for VM_OPTIONS_FILE in \
    ...
    $HOME/.AndroidStudio3.4/config/studio$BITS.vmoptions \
    $IDE_HOME.vmoptions \
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=AndroidStudio3.4 \
